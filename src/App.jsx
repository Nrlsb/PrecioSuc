...
// --- Componente de Configuración Admin (Tasas de Cambio) ---
function AdminSettings({ adminUsername }) {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/api/admin/settings?adminUsername=${adminUsername}`);
      const data = await response.json();
      if (response.ok) {
        setSettings(data);
      }
